...
class Factory(object):
    ...
    def __call__(self, **kwargs):
        res = self._eval_arguments(self._defaults, exclude=set(kwargs.keys()))
    ...
    def _process_metafactory_arguments(self, d):
        res = {}
        for k, v in d.items():
            if isinstance(v, type) and issubclass(v, Factory):
                res[k] = mkgen(v())
            elif v is not DELETE:
                res[k] = v
        return res

    def _eval_arguments(self, d, exclude=()):
        res = {}
        for k, v in d.items():
            if k not in exclude:
                if isinstance(v, Gen):
